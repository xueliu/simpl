...
#ifdef SUR_CHR	/********** char message **********/
	const static int size = sizeof(SUR_NAME_ATTACH_CHR_REPLY);
	SUR_NAME_ATTACH_CHR_REPLY *reply;
#else			/********** binary message **********/
...
// build the reply message
#ifdef SUR_CHR	/********** char message **********/
	reply = (SUR_NAME_ATTACH_CHR_REPLY *)memArea;
	btosSI(SUR_NAME_ATTACH, reply->hdr.token, intWidth);
	btosSI(result, reply->result, intWidth);
#else			/********** binary message **********/
